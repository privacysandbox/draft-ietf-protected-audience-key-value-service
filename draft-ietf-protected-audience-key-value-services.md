...
`key id`, a `metadata(TODO: Add the metadata struct link of W3C spec here)`
for global configuration, and `compression groups`, which is a list of
compression group(TODO: Add the compression group struct link of W3C spec here).
...
The output is a `results` which is a list of result(TODO: Add W3C spec struct link).
...
1. For each `group` in `response["compressionGroups"]`:
    ...
    1. For each `partition` in `content`:
        ...
        1. Let `result` be an empty struct(TODO: Add W3C spec struct link).
        1. For each `output` in `partition["keyGroupOutputs"]`:
            ...
            1. Let `key value` be an struct(TODO: Add W3C spec struct link).
            1. For each `key` → `value` in `output["keyValues"]`:
                ...
                1. Set `key value`'s `key` to `value["value"]`.
